...
export async function minerWorkflow({
  ...
  const toolResponse = textContent ?? "Miner completed but produced no output.";
  console.log(`[Miner] Done — response length=${toolResponse.length} chars`);
  return { toolResponse, data: null };
}

export const minerSubagent = {
  agentName: agentConfig.agentName,
  description: agentConfig.description,
  workflow: minerWorkflow,
};
